...
from transformers import BertConfig, BertTokenizer, AutoModelForTokenClassification, Trainer, TrainingArguments, AutoTokenizer
from BertCRF import BertCrfForNer
...
def try_all(models, fp):
    ...
    for model_typ in models:
        ...
        config = BertConfig.from_pretrained(model_typ,num_labels=len(unique_tags))
        tokenizer = AutoTokenizer.from_pretrained(model_typ)
        model = BertCrfForNer.from_pretrained(model_typ, config=config)
