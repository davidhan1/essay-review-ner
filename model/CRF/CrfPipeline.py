...
class NER_TRF:
    def __init__(self,
                 model_checkpoint=r'D:\Study\Python_project\essay-review-NER-big-data\ckpt\bert-base-chinese_fp32_ckpt\checkpoint-320',
                 device=0):
        print('device', device)
        self.label2tag = {
            'LABEL_0': 'O',
        ...
        config = BertConfig.from_pretrained(model_checkpoint, num_labels=len(self.label2tag.items()))
        tokenizer = AutoTokenizer.from_pretrained(model_checkpoint)
        model = BertCrfForNer.from_pretrained(model_checkpoint, config=config)
        if device < 0:
            model = model.to('cpu')
        else:
            model = model.cuda()
